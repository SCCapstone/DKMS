...
export const LikeButton = ({filled = false}: {filled?:boolean}) => {
    const [state, setState] = useState(styles.likeButton);
    ...
    const onClick = () => {
        if(state === styles.likeButtonClicked) {
            setState(styles.likeButton);
        } else {
            setState(styles.likeButtonClicked);
        }
    };

    return (
        <svg onClick={onClick} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={state}>
